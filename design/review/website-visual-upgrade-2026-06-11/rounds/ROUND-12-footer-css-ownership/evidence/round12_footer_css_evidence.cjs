...
const baseUrl = process.env.ROUND12_BASE_URL || "http://localhost:3001";
...
async function inspectPage(page, item) {
  return page.evaluate(
    ({ expectedLinks, expectedText, footerExpected, globeNAReason }) => {
      ...
      const moduleClassPresent = footerLinks.some((link) => String(link.className).includes("Footer-module"));
      const legacyFooterClassesPresent = Boolean(footer?.className.includes("footer-")) || footerLinks.some((link) => String(link.className).includes("footer-"));

      return {
        url: window.location.href,
        title: document.title,
        bodyChars: bodyText.length,
        nonEmptyRender: bodyText.length > 120,
        ...
        hydrationWarningInDom: /Hydration failed|hydration|did not match/i.test(bodyText),
        pageErrorInDom: /Unhandled Runtime Error|Application error|Build Error|Script error/i.test(bodyText),
        ...
        footerLinks,
        linkExpectations,
        allExpectedLinksFound: footer ? linkExpectations.every((link) => link.found) : !footerExpected,
        minTapHeight: footerLinks.length ? Math.min(...footerLinks.map((link) => link.height)) : null,
        minTapWidth: footerLinks.length ? Math.min(...footerLinks.map((link) => link.width)) : null,
        tapTargetsPass: footer ? footerLinks.every((link) => link.height >= 43 && link.width >= 44) : !footerExpected,
        moduleClassPresent,
        legacyFooterClassesPresent,
      };
    },
    {
...
async function captureCase(browser, item) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (!inspection.expectedTextFound && attempt < 3) {
  ...
  const footerPass = item.route.footerExpected
    ? inspection.footerFound &&
      ...
      inspection.moduleClassPresent &&
      !inspection.legacyFooterClassesPresent
    : !inspection.footerFound;
  const pass =
    inspection.nonEmptyRender &&
    inspection.expectedTextFound &&
    footerPass &&
    !inspection.frameworkOverlay &&
    !inspection.horizontalOverflow &&
    !inspection.hydrationWarningInDom &&
    !inspection.pageErrorInDom &&
  ...
  return {
    ...
    jsonResult: {
      url: inspection.url,
      title: inspection.title,
      nonEmptyRender: inspection.nonEmptyRender,
      footerStatus: inspection.footerStatus,
      frameworkOverlay: inspection.frameworkOverlay,
      relevantConsoleWarningsErrors: relevantConsoleMessages,
      hydrationWarnings,
      pageErrors,
...
async function runMatrix() {
  ...
  await browser.close();
  fs.writeFileSync(path.join(evidenceDir, "footer-css-visual-matrix-results.json"), JSON.stringify(results, null, 2));
  return results;
}
...
async function runInteractionAccessibility() {
  ...
  const staticMetrics = await page.evaluate((expectedLinks) => {
    ...
    return {
      ...
      moduleClassPresent: links.some((link) => String(link.className).includes("Footer-module")),
      legacyFooterClassesPresent: Boolean(footer.className.includes("footer-")) || links.some((link) => String(link.className).includes("footer-")),
    };
  }, expectedFooterLinks);
  ...
  const pass =
    ...
    staticMetrics.moduleClassPresent &&
    !staticMetrics.legacyFooterClassesPresent &&
    focusPathPass &&
    relevantConsoleMessages.length === 0 &&
    pageErrors.length === 0 &&
    hydrationWarnings.length === 0;
  ...
  fs.writeFileSync(path.join(evidenceDir, "footer-css-interaction-accessibility-results.json"), JSON.stringify(result, null, 2));
  return result;
}

(async () => {
  const matrixPath = path.join(evidenceDir, "footer-css-visual-matrix-results.json");
  ...
  fs.writeFileSync(path.join(evidenceDir, "footer-css-evidence-summary.json"), JSON.stringify(summary, null, 2));
