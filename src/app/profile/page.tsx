...
const TRUST_LABELS: Record<number, { label: string; color: string }> = {
  ...
  5: { label: "Legend", color: "bg-amber-100 text-[#E8824C]" },
};
...
export default function ProfilePage() {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-[#FFF8F0] to-[#FFFCF5] pt-24 pb-16 overflow-hidden">
      ...
      <div className="absolute bottom-[15%] right-[4%] w-20 h-10 text-[#F5A623]/8 doodle-float hidden lg:block" style={{ animationDelay: '3s' }}><DoodleBone className="w-full h-full" /></div>
      <div className="max-w-4xl mx-auto px-4">
        {/* ── Profile Header Card ── */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-white rounded-3xl shadow-sm border border-[#F5E6D3]/60 overflow-hidden mb-6"
        >
          {/* Banner */}
            ...
            <AnimatePresence mode="wait">
              {editing ? (
                ...
                <motion.div key="edit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                  ...
                  <div className="flex gap-2 pt-1">
                    <button onClick={handleSave} disabled={saving}
                      className="flex items-center gap-1.5 px-5 py-2.5 bg-[#F5A623] hover:bg-[#E8824C] disabled:opacity-50 text-white rounded-xl text-sm font-bold transition-all shadow-sm">
                ...
                <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <h1 className="text-2xl font-extrabold text-[#1E293B]">{profile.display_name || profile.username}</h1>
                  <p className="text-sm text-[#6B7B8D]/70">@{profile.username}</p>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            { icon: PawPrint, label: "Pets", value: pets.length, color: "text-[#F5A623]" },
            ...
            <motion.div key={label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              className="bg-white rounded-2xl shadow-sm border border-[#F5E6D3]/60 px-5 py-4 text-center">
              <Icon className={`w-5 h-5 mx-auto mb-1 ${color}`} />
              <p className="text-xl font-extrabold text-[#1E293B]">{value}</p>
              <p className="text-xs text-[#6B7B8D]/70 font-medium">{label}</p>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-[#1E293B] flex items-center gap-2">
              <PawPrint className="w-5 h-5 text-[#F5A623]" /> My Pets
            </h2>
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold transition-all border shadow-sm ${
                showAddPet
                  ...
                  : "bg-[#F5A623] hover:bg-[#E8824C] text-white border-amber-500"
              }`}
            >
          ...
          <AnimatePresence>
            {showAddPet && (
              ...
              >
                <div className="bg-white rounded-2xl shadow-sm border border-[#F5E6D3]/60 p-6 space-y-5">
                  {/* Pet Name */}
                  <div>
                  ...
                  {/* Species */}
                  <div>
                    <label className="text-xs font-bold text-[#6B7B8D]/70 uppercase tracking-wider">Species</label>
                  ...
                  {/* Social Tags */}
                  <div>
                    <label className="text-xs font-bold text-[#6B7B8D]/70 uppercase tracking-wider">Personality Tags</label>
                  ...
                  {/* Health Badges */}
                  <div>
                    <label className="text-xs font-bold text-[#6B7B8D]/70 uppercase tracking-wider">Health &amp; Care</label>
                    <div className="flex flex-wrap gap-1.5 mt-2">
                  ...
                  <button onClick={handleAddPet} disabled={addingPet || !petName.trim()}
                    className="w-full flex items-center justify-center gap-2 px-5 py-3 bg-[#F5A623] hover:bg-[#E8824C] disabled:opacity-50 text-white rounded-xl text-sm font-bold transition-all shadow-sm">
          ...
          {pets.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-sm border border-[#F5E6D3]/60 px-6 py-12 text-center">
              <PawPrint className="w-12 h-12 text-gray-200 mx-auto mb-3" />
              <p className="text-[#6B7B8D]/70 font-medium">No pets yet</p>
              ...
              {pets.map((pet, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className="bg-white rounded-2xl shadow-sm border border-[#F5E6D3]/60 overflow-hidden hover:shadow-md transition-shadow"
                  >
                    <div className="flex items-start gap-4 p-5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="text-base font-bold text-[#1E293B]">{pet.name}</h3>
                          {pet.nfc_tag_uid && (
                        ...
                        {quirk && <p className="text-xs text-[#6B7B8D]/70 italic mt-0.5">&ldquo;{quirk}&rdquo;</p>}
                      ...
                      <a
                        href={`/tag/${pet.id}`}
                        target="_blank"
                        className="shrink-0 self-center text-gray-300 hover:text-[#F5A623] transition-colors"
